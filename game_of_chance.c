...
int get_palyer_data(){
    ...
    read_bytes = read(fd,&entry, sizeof(struct user)); //Read fisrt chunck.
    while(entry.uid !=uid && read_bytes >0) {//Loop til proper uid found.
        read_bytes = read(fd,&entry,sizeof(struct user)); //Keep reading.
    }

    close(fd);

    if (read_bytes < sizeof(struct user))//File has reach the end
        return -1;
    else 
        player = entry;
    return 1;
...
// writes current player data to File
...
//Award the jackpot for the Pick a Number game_of_chance.c
void jackpot() {
    printf("*+*+*+*+*+* JACKPOT *+*+*+*+*+*\n");
...
int dealer_no_match() {
    ...
    if(match!=-1){
        ...
        printf("There are no matches! You win %d credits!\n",wager);
        player.credits +=wager;
    }
